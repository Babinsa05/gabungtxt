...
// ==================== SUPABASE CONFIG ====================
const SUPABASE_URL = 'https://yyeagjiyvaqtjmaltapl.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inl5ZWFnaml5dmFxdGptYWx0YXBsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE0MDcxMTQsImV4cCI6MjA5Njk4MzExNH0.c4UOWUSNaSzfrYoW_KDVQtZtjfLeBD9K59mPwKhnYBs';
...
async function simpanCSSkeSupabase(namaFile, kontenCSS) {
    const { error } = await supabase
        .from('file_css')
        .upsert({ id: namaFile, konten: kontenCSS, updated_at: new Date() });
    if (error) { console.error('Gagal simpan CSS:', error); return false; }
...
async function bacaCSSdariSupabase(namaFile) {
    const { data, error } = await supabase
        ...
        .eq('id', namaFile)
        .single();
    if (error) { console.error('Gagal baca CSS:', error); return null; }
...
async function simpanHasilGabungan(hasilTeks, metadata) {
    ...
    if (error) { console.error('Gagal simpan hasil:', error); return false; }
    console.log('✅ Hasil gabungan tersimpan');
    return true;
}
...
async function ambilRiwayatHasil(limit = 10) {
    const { data, error } = await supabase
        ...
        .limit(limit);
    if (error) { console.error('Gagal ambil riwayat:', error); return []; }
    return data;
}
...
async function simpanFileTXT(namaFile, konten, ukuran) {
    ...
    if (error) { console.error('Gagal simpan file:', error); return false; }
    return true;
}
...
document.getElementById('saveToSupabaseBtn').onclick = async () => {
    const cssKonten = await fetch('style.css').then(r => r.text()).catch(() => {
        let css = '';
        document.querySelectorAll('style').forEach(style => { css += style.textContent; });
        return css;
    });
...
document.getElementById('loadCssFromSupabaseBtn').onclick = async () => {
    const success = await terapkanCSSdariSupabase();
    showToast(success ? '✅ CSS dimuat!' : '⚠️ Gagal load CSS', success ? '#10b981' : '#f59e0b');
};

document.getElementById('historyBtn').onclick = async () => {
    const riwayat = await ambilRiwayatHasil(10);
    if (riwayat.length === 0) { showToast('Belum ada riwayat', '#475569'); return; }
